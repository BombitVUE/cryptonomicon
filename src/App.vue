          ...
          <div class="max-w-xs">
            ...
            <div class="mt-1 relative rounded-md shadow-md">
              <input
                ...
                v-on:input="filteredCoins = filterCoins(ticker)"
                v-on:blur="
                  resetError;
                  resetFilter;
                "
...
export default {
  ...
  setup() {
    ...
    const filterCoins = (query) => {
      ...
      const filteredCoins = coins.value.filter((coin) => {
  ...
  computed: {
    ...
    errorMessage() {
      const tickerExists = this.tickers.some((t) => {
        return t.name.toLowerCase() === this.ticker.toLowerCase();
      });
      return tickerExists ? "Такой тикер уже добавлен" : "";
    },
    pageStateOptions() {
      return {
        filter: this.filter,
  ...
  methods: {
    ...
    add() {
      const tickerExists = this.tickers.some(
